...
const CommodityIcon: React.FC<CommodityIconProps> = ({ commodity, size = 20, className = '' }) => {
  ...
  const combinedClassName = `commodity-icon ${className}`.trim();

  switch (commodity) {
    case 'soja':
      return (
        <svg style={iconStyle} className={combinedClassName} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
